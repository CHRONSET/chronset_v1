...
for jt = 1:length(ID)
    ...
    [id] = [raw{jt,3},'.',num2str(raw{jt,4}),'_',chck.date(1:12),'.mat'];    
...
parfor it = 1:length(ID2)
    ...
    dat(it,:,:) = [dum.savedata(3,2) dum.savedata(3,3);dum.savedata(4,2) dum.savedata(4,3)];
...
%%
path2files = '/bcbl/home/home_a-f/froux/chronset/data/SayWhen/feature_data/';
...
%     x = cell(5,5);
%     for jt = 1:size(dum.savedata,1)
%         for kt = 1:size(dum.savedata,2)
%             x{jt,kt} = dum.savedata{jt,kt};
%             x{jt,kt}.id = ID2{it};
%         end;
%     end;
