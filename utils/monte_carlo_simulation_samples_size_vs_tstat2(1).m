...
%in clara's paper, the cognate effect had an F = 127.1, which works out
% to a t of 11.7.  Our t-value is higher than that, likely, for several
% reasons, including that we are dealing with a perfectly normal case,
% we do not have between-group noise (from the monolinguals + bilinguals)
% etc.  
...
N = 5:80;% number of participants (sample size)
        %trick with N-start=9 to get label for 10 on plot
...
for it = 1:n_iter%loop over number of emulations
    ...
    for jt = 1:length(N)% loop over samples sizes        
        ...
        for kt = 1:N(jt)% simulate n observations for each individual participant
            ...
            x2(kt,:) = v2;
            % noise added
            x3(kt,:) = v1+S1_chron1*randn(1,n);            
            x4(kt,:) = v2+S1_chron1*randn(1,n);
        ...
        x6 = x4'-x3';
        [h,p,ci,stats] = ttest(x6,0);% noise added 
...
for it = 1:N(end)
    ...
    x3(it,:) = M1+SD2*randn(1,n)+S1_chron1*randn(1,n); 
    x4(it,:) = M1+SD2*randn(1,n)+S1_chron1*randn(1,n);
end;
